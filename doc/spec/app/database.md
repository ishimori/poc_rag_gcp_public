...
評価パイプラインの実行結果を保存する。Firestore保存 + ローカルファイル保存（フォールバック）の二重保存。

### フィールド定義

| フィールド | 型 | 必須 | 説明 |
|-----------|---|------|------|
| `date` | string | Yes | 評価実行日時 |
| `config_params` | map | Yes | 評価時のパラメータ（chunk_size, chunk_overlap, header_injection, top_k, rerank_top_n, rerank_threshold） |
| `overall` | map | Yes | 全体スコア（total, passed, failed, score） |
| `score_by_type` | map | Yes | テストタイプ別スコア |
| `failed_cases` | array | Yes | 失敗ケースの詳細 |
| `timestamp` | timestamp | Yes | サーバータイムスタンプ |

### 書き込み

- **タイミング**: 評価実行（`POST /api/admin/evaluate`）完了時
- **実装**: `src/evaluate/reporter.py` — `save_report()`
- **フォールバック**: Firestore保存失敗時はローカルファイル（`results/`）のみに保存

### 読み取り

- **デフォルトソート**: `date` DESC
- **上限**: 50件
- **フォールバック**: Firestore読み取り失敗時はローカルファイルから読み込み
...
> `eval_results` は独立したコレクション（他コレクションへの参照なし）。
