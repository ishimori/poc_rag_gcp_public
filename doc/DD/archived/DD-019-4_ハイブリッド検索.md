...
### Phase 2: パイプライン接続 ✅
- [x] `src/search/flow.py`: Step 1を変更
...
- [x] `src/search/hybrid.py`: `user_groups`引数を追加（`vector_search`への引き回し）
- [x] `src/ingest/store.py`: `store_chunks()`と`clear_collection()`の末尾で`invalidate_chunk_cache()`を呼び出し（Ingest後にキーワード検索キャッシュが古くならないようにする）
- [x] `src/evaluate/scorer.py`: `FEATURE_MAP`に`"hybrid_search": "hybrid_search"`を追加（`"contextual_retrieval"`の下に追記）
- [x] 🔬 **機械検証**: syntax OK（4ファイル） + `ruff check` All checks passed
- [x] 😈 **DA批判レビュー**: 下記Phase 2 DA記録に詳細。ログ二重出力（許容）、ingest→search依存（許容）、import時初期化（遅延初期化で問題なし）

### Phase 3: 評価・記録 ✅
- [x] 簡易評価結果（`--limit 10`）をDD-019-4ログに記録
- [x] DD-019の子DD一覧にステータス・スコア変動を更新
- [x] 👀 **目視確認**: exact_matchテスト項目のスコアが改善しているか確認
...
- Phase 2完了:
  - `src/search/flow.py`: Step 1を`config.hybrid_search`で分岐、`[HybridSearch]`/`[VectorSearch]`ログ出力
  - `src/search/hybrid.py`: `user_groups`引数を追加（`vector_search`への引き回し）
  - `src/ingest/store.py`: `store_chunks()`/`clear_collection()`末尾で`invalidate_chunk_cache()`呼び出し
  - `src/evaluate/scorer.py`: `FEATURE_MAP`に`hybrid_search`追加
  - syntax + ruff lint全パス
  - DA: ログ二重出力・ingest→search依存・import時初期化の3件、いずれも許容
- Phase 3完了（簡易評価 --limit 10）:
  - exact_match: **4/5 (80.0%)** ← 0%から大幅改善
  - similar_number: **3/3 (100.0%)** ← 33%から改善
  - semantic: 0/2 (0%) ← 変化なし（キーワード検索の対象外）
  - 全体: **7/10 (70.0%)**
  - exact-005（999997の在庫状況）はLLMが空レスポンス → ハイブリッド検索は正しく動作（keyword=3 hits）、LLM側の問題
  - semantic-001/002はキーワード検索が空リスト → ベクトル検索のみにfallback（設計通り）
  - `[HybridSearch]`ログ正常出力確認
...
**DA観点:** パイプライン統合時の副作用・アーキテクチャ影響

| # | 発見した問題/改善点 | 重要度 | 再現手順（高/中は必須） | DA観点 | 対応 |
|---|-------------------|--------|----------------------|--------|------|
| 1 | `hybrid_search()`内のログ`[HybridSearch] vector=X, keyword=Y, merged=Z`と`flow.py`の`[HybridSearch] N results`が二重出力 | Low | ハイブリッド検索ONでクエリ実行 → コンソールに`[HybridSearch]`が2行 | ログ冗長 | ⏭️許容: hybrid.pyは内部詳細（vector/keyword件数）、flow.pyは全体フロー表示で役割が異なる |
| 2 | `store.py`が`keyword_searcher`をimportすることでingest→searchの依存が発生 | Medium | — | アーキテクチャ | ⏭️許容: 一方向依存で循環なし。`invalidate_chunk_cache()`は副作用のないキャッシュ破棄のみ。本番化時はイベントバスに置き換え可能 |
| 3 | `config.hybrid_search=False`でも`from src.search.hybrid import hybrid_search`のimportが走る | Low | hybrid_search=Falseで起動 → importは走るがFirestoreクライアント初期化は遅延 | パフォーマンス | ⏭️許容: モジュールimportのみ。DBアクセスは`_get_db()`の遅延初期化で関数呼び出し時まで発生しない |
