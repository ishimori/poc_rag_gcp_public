...
## クイックスタート: Vertex AI Ranking API

### 前提条件

- GCPプロジェクトで Discovery Engine API を有効化済み
...
def rerank(
    ...
    """Vertex AI Ranking API でチャンクを再順位付けする"""
    client = discoveryengine.RankServiceClient()

    # チャンクを RankingRecord に変換
    records = [
    ...
    return [
        {
            "content": chunks[int(record.id)]["content"],
            "metadata": chunks[int(record.id)].get("metadata", {}),
            "score": record.score,
        }
...
if __name__ == "__main__":
    search_results = [
        {"content": "ネジ999999の材質はSUS304...", "metadata": {"doc_title": "ネジ仕様書"}},
        {"content": "VPNの設定手順は...", "metadata": {"doc_title": "ITヘルプ"}},
        {"content": "ネジ999999の公差は±0.01mm...", "metadata": {"doc_title": "品質基準書"}},
    ]
    ranked = rerank("your-project-id", "ネジ999999の公差", search_results)
    for r in ranked:
        print(f"[{r['score']:.3f}] {r['content'][:60]}")
```

!!! tip "Discovery Engine API の有効化"
    Ranking API は Vertex AI Search の一部として提供されている。GCPコンソール → APIとサービス → Discovery Engine API を有効化する。
    [公式ガイド: Ranking API](https://cloud.google.com/generative-ai-app-builder/docs/ranking)
