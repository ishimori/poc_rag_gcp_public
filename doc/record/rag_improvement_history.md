...
>
> **最終更新: 2026-03-22（DD-025 Gemini 3 Flash 評価完了・精度追求中止決定）**
...
```mermaid
xychart-beta
    title "RAG Overall Score 推移（代表値）"
    x-axis ["Baseline", "GrpA", "GrpB", "019-8", "019-8-2", "019-10/13", "chunk=1200", "Vertex素", "3Flash×Vertex"]
    y-axis "Overall %" 0 --> 100
    bar [28.4, 34.8, 50.0, 75.7, 81.1, 81.1, 85.1, 84.4, 79.7]
    line [28.4, 34.8, 50.0, 75.7, 81.1, 81.1, 85.1, 84.4, 79.7]
```
...
timeline
    ...
    section DD-022 チャンク再実験
        chunk=600 → 77.0%（バグ修正後、小チャンクが最適と判明）
        : 4コレクション並列実験（800/1200/1600/2000）
        : ⚠ Shadow Retrieval誤発動バグ → hybrid同士の比較に修正
        : DD-019-9の「大きいほど良い」結論が覆る
    section DD-024 Vertex AI Search
        Vertex素 84.4%（チューニングなし）
        : 自前RAGと同等精度をout-of-boxで達成
        : ⚠ 制御層そのまま載せると逆効果（83.8%）
    section DD-024-3 Phase1 + DD-025 3Flash評価
        3 Flash は全パターンで2.5 Flashより低下（-4〜9pt）
        : Wikipedia Boostで noise_resistance改善
        : ❌ 3 Flash: unanswerable/cross_category 悪化
        : 🏁 精度追求中止。現行データセットで85%前後が天井
```
...
| カテゴリ | ベースライン | GrpA | GrpB | DD-019-8 | DD-019-8-2 | DD-019-10/13 | Vertex素(A) | 自前(chunk=800)×3Flash |
|---------|-----------|------|------|----------|------------|--------------|:-----------:|:---------------------:|
| exact_match (8) | 0% | 0% | **75%** | **100%** | 100% | 88% | **100%** | 100% |
| similar_number (6) | 33% | 33% | **83%** | **100%** | 100% | 100% | 83% | 100% |
| semantic (12) | 17% | 17% | 17% | **67%** | 67% | 50% | **67%** | 33% |
| step_sequence (7) | 71% | 71% | 71% | **100%** | 100% | 100% | **100%** | 86% |
| multi_chunk (7) | 29% | 29% | **71%** | **100%** | 100% | 100% | 86% | 71% |
| unanswerable (5) | 100% | 100% | 100% | 80% | 80% | **100%** | **100%** | 60% |
| ambiguous (5) | 0% | **60%** | 60% | 60% | 60% | **100%** | (skip) | 100% |
| cross_category (5) | 40% | 40% | 40% | **80%** | 80% | **100%** | **100%** | 60% |
| security (5) | — | (skip) | 0% | 0% | 0% | **20%** | (skip) | 100% |
| noise_resistance (6) | 33% | 33% | **50%** | 67% | 83% | 67% | 50% | 83% |
| table_extract (5) | 20% | 20% | 20% | **80%** | 80% | **100%** | 80% | 100% |
| temporal (3) | 0% | 0% | 0% | 33% | **100%** | 67% | **100%** | 67% |
...
**注意**: 実験A/BはDD-019-10/13の施策が同時適用された状態。後のDD-022でバグ修正後に再実験した結果、**この結論（大きいほど良い）は覆った**（下記参照）
...
| パラメータ | 値 | 備考 |
|---|---|---|
| chunk_size | 800 | DD-022で600が最適と判明（800はsecurity 100%の利点あり） |
| chunk_overlap | 150 | |
| hybrid_search | true | |
...
### DD-022: チャンクサイズ実験（バグ修正後の公平な再実験）

**変更した軸**: チャンクサイズ

**経緯**: DD-019-14 の4段階バグ修正後、同一コード状態で6パターン（200/400/600/800/1200/1600/2000）を並列 Ingest + Evaluate。DD-019-9 の「大きいほど良い」結論を検証した。

| chunk_size | overlap | chunks | Overall | semantic | security | unanswerable | table_extract |
|-----------|---------|--------|---------|----------|----------|-------------|--------------|
| **600** | 100 | 276 | **77.0%** | **58%** | 20% | **100%** | 60% |
| **400** | 66 | 289 | **77.0%** | **58%** | 40% | **100%** | 60% |
| 800 | 150 | 403 | 74.3% | 42% | **100%** | 60% | **100%** |
| 1200 | 200 | 292 | 73.0% | 42% | 80% | 60% | 80% |
| 1600 | 300 | 231 | 71.6% | 42% | 80% | 40% | 60% |
| 2000 | 400 | 185 | 66.2% | 42% | 80% | 20% | 60% |
| 200 | 33 | — | 中止 | — | — | — | — |

**結論**:
- **DD-019-9 の結論（大きいほど良い）は覆った** — バグ修正後は小さいチャンクの方が高精度
- スイートスポットは **chunk_size=600** 付近
- 小チャンクの強み: semantic +16pt, unanswerable +40pt（情報が絞られて検索精度向上）
- 小チャンクの弱み: security が低い（600=20% vs 800=100%）— 権限メタデータが分断される可能性
- 400 と 600 はほぼ同等。200 は遅すぎて実用外

**失敗・勘違い:**

1. **Shadow Retrieval 誤発動バグ** — `flow.py` で hybrid 結果と vector 結果を比較していたため、検索方式の違いでほぼ全クエリで「権限除外」と誤判定。初回評価が 16〜21% という異常値で発覚。hybrid 同士で比較するよう修正
2. **Firestore インデックス未作成** — 新コレクションには自動でインデックスが作られない。初回 Evaluate 時に `Missing vector index configuration` エラー。事前にインデックスを作る運用ルールを手順書に追記
...
## DD-024-3 Phase 1: Wikipediaノイズ抑制（コード側 Boost）

**変更した軸**: 検索技術（Vertex AI Search 経路のみ）

GCS パスの `wikipedia` 文字列でノイズ文書を判別し、スコアに 0.3 のペナルティを適用。`page_size` を `top_k × 2` に拡大してリランク。

**効果**: DD-025 の Vertex × 3 Flash 評価で noise_resistance が 50% → 83.3% に改善（DD-024 比 +33pt）

**失敗・勘違い:**
1. **API-level Boost 不可** — Vertex のデータストアが `dataSchema: "content"` のためメタデータフィールドなし → コード側のpost-retrieval boostで代替。Phase 3 で権限メタデータが必要になった時にまとめて再検討

---

## Gemini 3 Flash モデル評価（DD-025）

**変更した軸**: 回答生成 LLM（gemini-2.5-flash → gemini-3-flash-preview）

**LLM-as-Judge は据え置き**（過去スコアとの公平な比較のため）
...
1. **旧SDK (`vertexai`) では Gemini 3 系が呼べない** — `asia-northeast1` で 404。`google.genai` 新SDK + `global` リージョンに移行
2. **unanswerable/cross_category が悪化** — Shadow Retrieval の誤発動で「権限なし」誤回答が増加。clarifier プロンプトとモデル特性の変化が絡んでおり原因未特定

### 精度追求中止決定（2026-03-22）

DD-025 の評価結果から以下の知見が得られ、精度追求を中止した:

1. 検索エンジンを変えてもスコアは変わらない（自前RAG 85% ≈ Vertex 84%）
2. LLMモデルを上げてもスコアは改善しない（3 Flash が全パターンで低下）
3. 制御層の個別調整でも最大 +5pt 程度（コスト対効果が低い）
4. 評価コストが高額（74件×回答生成+LLM-as-Judge が蓄積）

**結論: 現行の評価データセット・文書セットでは 85% 前後が実質的な天井。DD-027（PoC報告書）に舵を切る。**
...
| chunk_size | 800（config.py デフォルト） | DD-022 で 600 が最適と判明。security 問題を解決すれば 80%+ が見込める |
