...
| [05](05_evaluation.md) | **自動評価パイプライン** | ✅ 実装済み | 74件のテスト質問（12パターン）で精度を自動測定 |
...
テストデータはLLM（Claude）を使って自動生成しました。社内文書19件に加え、ノイズ源としてWikipedia日本語版から42記事を投入し、実運用に近い「大量の候補の中から正解を見つける」条件で評価しています。

| 項目 | 内容 |
|------|------|
| テスト用社内文書 | 19件（IT FAQ、VPN、部品仕様書×4、品質管理基準書、経費精算、休暇規程など） |
| ノイズ用Wikipedia記事 | 42件（ステンレス鋼、ボルト、VPN、労働基準法、減価償却など関連トピック） |
| 検索対象チャンク数 | 454（社内51 + Wikipedia403） |
| テスト質問数 | **74件**（**12パターン**） |
| スコアリング方法 | キーワード含有判定（LLMベースの意味評価は今後導入予定） |

### テストの内容 — 12種類の質問パターン

| テスト種別 | 件数 | テスト質問の例 | 期待する回答 |
|-----------|------|-------------|-------------|
| **完全一致の検索** | 8 | 「ネジ999999の材質は？」 | 「SUS304」 |
| **似た番号の区別** | 6 | 「ネジ999998の材質は？」 | 「SUS316L」（999999のSUS304と混同しないこと） |
| **意味の検索** | 12 | 「PCが重い」 | メモリ確認→不要アプリ終了→再起動の手順 |
| **手順の正確さ** | 7 | 「VPN接続の手順3の後は何をする？」 | 手順4のタスクバー緑色確認の説明 |
| **複数文書の統合** | 7 | 「ネジ999999と999998の違いは？」 | 2つの仕様書を横断した比較表 |
| **答えられない質問** | 5 | 「来月の株価はどうなる？」 | 「情報が見つかりません」 |
| **曖昧な質問** | 5 | 「エラーが出る」 | 「どのシステムで？エラーメッセージは？」と聞き返す |
| **分野横断の質問** | 5 | 「有給の申請方法とVPNの設定を教えて」 | 人事規程とVPNマニュアルの両方から回答 |
| **権限制御** | 5 | 「給与テーブルを見せて」 | 「アクセス権限がありません」 |
| **ノイズ耐性** | 6 | 「SUS304の特性を教えて」 | Wikipediaではなく社内仕様書から回答 |
| **表の読み取り** | 5 | 「5万円の経費は誰が最終承認する？」 | 承認権限テーブルから「本部長」 |
| **新旧文書の区別** | 3 | 「有給の繰越上限は何日？」 | 旧版（10日）ではなく現行版（20日）を回答 |

### LLM生成テストによるベースライン: 21/74問 正解（28.4%）

> **注**: このスコアはLLMが生成したテストデータと、キーワード含有による自動判定の結果です。テスト条件・判定方法ともに改善途上であり、RAGシステムの最終的な精度を示すものではありません。

| テスト項目 | 何を測っているか | 結果 | 状態 | 改善策 |
|-----------|----------------|------|------|--------|
| 完全一致の検索 | 型番で正確に引けるか | 0/8 | 要改善 | キーワード検索併用（06） |
| 似た番号の区別 | 999999と999998を混同しないか | 2/6 | 要改善 | キーワード検索併用（06） |
| 意味の検索 | 言い換え・類義語に対応できるか | 2/12 | 要改善 | 文脈説明の自動付与（10） |
| 手順の正確さ | 手順を正しい順序で回答するか | **5/7** | 概ね良好 | — |
| 複数文書の統合 | 複数ファイルをまたいで回答できるか | 2/7 | 要改善 | 文脈説明の自動付与（10） |
| 答えられない質問 | 根拠がないときに「分かりません」と言えるか | **5/5** | **完璧** | — |
| 曖昧な質問 | 情報不足のとき聞き返せるか | 0/5 | 未実装 | 聞き返し機能（07） |
| 分野横断の質問 | 複数分野から情報を集められるか | 2/5 | 要改善 | インテントルーティング（13） |
| 権限制御 | 閲覧権限外の情報を出さないか | 0/5 | 未実装 | 権限フィルタ（08） |
| ノイズ耐性 | Wikipediaノイズの中から正解を選べるか | 2/6 | 要改善 | メタデータスコアリング（11） |
| 表の読み取り | 表の行列交差情報を正確に抽出できるか | 1/5 | 要改善 | チャンキング改善（01） |
| 新旧文書の区別 | 最新版の情報を正しく参照できるか | 0/3 | 要改善 | セルフクエリ（12） |

**この数字の読み方:**

- **スコアが低いのは「テストが難しい」から。** 454チャンク（うち403件がWikipedia）の中から正解の社内文書を見つける必要があり、実運用に近い厳しい条件です
- **「答えられない質問」が100%** — RAGの基本（ハルシネーション抑制）は正しく機能しています
- **「手順の正確さ」が71%** — VPN設定など手順系の質問には堅牢に回答できます
- **0%のカテゴリは「未実装の機能」** — 曖昧質問（07）と権限制御（08）はまだ作っていないだけです
- **各テスト項目に対応する改善技術（カッコ内の番号）がすべて特定済み**です

### 改善の見通し

重要なのは、**「何が弱いか」と「どの技術で改善するか」が数値で紐づいている**点です。

```mermaid
graph LR
    A[テスト実行<br/>74件×12パターン] --> B[弱いパターンを特定]
    B --> C[対応する技術を実装]
    C --> D[再テストで効果確認]
    D --> A
```

この**「測って、特定して、対策して、再測定する」サイクル**を管理画面からワンクリックで回せる仕組みが構築済みです。
...
[📋 概要](00_project-overview.md) | [01. 文書の分割 →](01_chunking.md)
