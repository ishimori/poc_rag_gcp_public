...
| 原因 | 具体例 | 本プロジェクトの対策 |
|------|--------|-------------------|
| **①切り方が悪い** | 「材質: SUS304」と「用途: 食品機械」が別チャンクに分断される | 意味のまとまりで分割 + 文書タイトルを各断片に付与（→ [詳細](01_detail-chunking.md)） |
| **②検索が雑** | 「PCが重い」で検索しても「パソコンの動作が遅い場合」がヒットしない | 意味の近さで検索 + キーワード検索の併用（→ [詳細](02_detail-search.md)） |
| **③渡す情報の質が低い** | 10件の検索結果のうち3件しか関係ないが、全部AIに渡してしまう | AIが関連度を再評価して上位だけに絞る（→ [詳細](03_detail-reranking.md)） |
...
- **曖昧な質問には聞き返す** — 情報が足りないときに的外れな検索をせず、「OSは何ですか？」等と確認する（→ [詳細](04_detail-ai-behavior.md)）
- **権限に応じて検索対象を制限** — 検索の前に、そのユーザーが見てよい文書だけに絞る（→ [詳細](05_detail-security.md)）
- **精度を自動で測る仕組み** — 45件のテスト質問で変更のたびに精度を数値化（→ [詳細](06_detail-evaluation.md)）
...
| # | 対策 | やっていること | たとえ話 | 詳細 |
|---|------|-------------|---------|------|
| 1 | **文書の分割** | 意味のまとまりで文書を分割し、各断片に文書タイトルを付ける | 本をページでなく「章」で切り分け、表紙のコピーを各章に挟む | [詳細](01_detail-chunking.md) |
| 2 | **意味で探す検索** | 質問と文書を「意味の数値」に変換し、意味が近いものを探す | 「PCが重い」と「パソコンが遅い」が同じ意味だと分かる検索 | [詳細](02_detail-search.md) |
| 3 | **AIによる再評価** | 検索結果をAIが1件ずつ精密に読み直し、本当に関連するものだけに絞る | 図書館で10冊取ってきた本の目次を読んで、5冊に厳選する | [詳細](03_detail-reranking.md) |
| 4 | **AIの賢い振る舞い** | 曖昧な質問には聞き返し、複雑な質問には段階的に調べる | 「印刷できない」→「Windows? Mac? プリンタの機種は？」と確認 | [詳細](04_detail-ai-behavior.md) |
| 5 | **セキュリティ** | 検索前にアクセス権をチェック。見てはいけない文書はそもそも検索しない | 図書館の閉架書庫。許可証がないと棚にすら入れない | [詳細](05_detail-security.md) |
| 6 | **自動評価** | 45件のテスト質問で、変更のたびに精度を自動測定 | 模擬試験を何度でも受けてスコアの変動を確認 | [詳細](06_detail-evaluation.md) |
...
| 対策技術 | 状態 | 説明 |
|---------|------|------|
| **文書の分割（チャンキング）** | ✅ 実装済み | 意味のまとまりで分割。800トークンごとに150トークンの重なりを持たせる |
| **文書タイトルの付与** | ✅ 実装済み | 各断片の先頭に元文書のタイトル・カテゴリを付ける |
| **意味で探す検索（ベクトル検索）** | ✅ 実装済み | 質問と文書を768個の数値に変換し、意味の近さで検索 |
| **AIによる再評価（リランキング）** | ✅ 実装済み | 検索結果をAIが精密に再評価して上位5件に絞る |
| **自動評価パイプライン** | ✅ 実装済み | 45件のテスト質問で精度を自動測定 |
| キーワード検索との併用 | 📋 調査済み | 型番検索の弱点を補う。**次の実装候補** |
| 曖昧な質問への聞き返し | 📋 調査済み | プロンプト改善で対応可能 |
| 検索前の権限フィルタリング | 📋 調査済み | メタデータ項目は保存済み。フィルタ処理を追加予定 |
| LLMによる品質評価 | 📋 調査済み | キーワード判定より精密な自動評価 |
| 文脈説明の自動付与 | 📋 調査済み | AIが各断片に固有の文脈説明を生成（分割精度の大幅向上が見込める） |
| メタデータによるランキング改善 | 📋 調査済み | 文書の更新日やカテゴリをランキングに反映 |
| AIによるフィルタ自動生成 | 📋 調査済み | 「2024年以降の人事規程」→ 条件を自動抽出して検索を絞る |
| 質問の種類に応じた検索切替 | 📋 調査済み | IT質問、部品検索、規程確認でそれぞれ最適な検索方法に切り替え |
