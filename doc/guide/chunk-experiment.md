...
```bash
# Evaluate のみ実行（Ingest 済みコレクション）
python scripts/run_experiment.py --collection chunks_1200 --evaluate-only &
python scripts/run_experiment.py --collection chunks_1600 --evaluate-only &
python scripts/run_experiment.py --collection chunks_2000 --evaluate-only &
```
...
| `scripts/evaluate.py` | Evaluate 単体（`--collection` 対応） |
